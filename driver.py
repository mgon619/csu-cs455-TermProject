...
def main():
    ...
    df_full = df_full.withColumn("RH Max", col("RH Max").cast("double")) \
    ...
    cutoff = df_fixed.agg(functions.percentile_approx(col("Liquid Precip"), 0.95).alias("cutoff")).collect()[0]["cutoff"]
    print(cutoff)
    #df_highprecip = df_fixed.filter(col("Liquid Precip") >= cutoff)
    df_fixed = df_fixed.withColumn(
        "High_Precip",
        functions.when(col("Liquid Precip") >= cutoff, 1).otherwise(0)
    )
    df_fixed.show()
